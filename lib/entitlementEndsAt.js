...
/** Applicant intro reels add-on: 12 calendar months (aligned with annual study plans). */
export function endsAtForReelsAddon(startDate = new Date()) {
  return addUtcMonths(asDate(startDate), 12);
}
...
export function extendReelsAddonEndsAt(previousEndsAtIso, now = new Date()) {
  const nowD = asDate(now);
  const prev = previousEndsAtIso ? asDate(previousEndsAtIso) : null;
  const base = prev && prev > nowD ? prev : nowD;
  return endsAtForReelsAddon(base);
}
